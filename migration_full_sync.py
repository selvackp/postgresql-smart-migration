...
logging.basicConfig(filename="migration_full_sync.log", level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
...
def load_config(config_file="config_full_load.yaml"):
...
def parse_args():
    parser = argparse.ArgumentParser(description="PostgreSQL smart full migration sync")
    parser.add_argument("--config", default="config_full_load.yaml", help="Path to YAML config file")
